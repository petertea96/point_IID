...
levels(most_likely_set_scores_df_plot$score) <- c('Other', levels(most_likely_set_scores_df_plot$score))
...
win_serve_plot <- 
  ...
  scale_fill_manual(values = c('#E67C7C', '#aaf0d1')) + 
...
( (match_scores_plot + match_win_plot) / (set_scores_plot + win_serve_plot)) + 
  ...
  plot_annotation(title = my_title,
                  subtitle = 'Championship Match Cincinnati 2021',
